...
const JustForYou = () => {
  ...
  const fetchProducts = async () => {
    try {
      // API call
      const response = await sellerauth.justForYouApi();

      if (response && response.success) {
        setProducts(response.products);
      } else {
        console.log("error:", response.message);
        toast.error(response.message || "No products found");
      }
    } catch (err) {
  ...
  if (loading) {
    return <div className="w-full  flex items-center justify-center"><div className="w-12 h-12 border-4 border-gray-300  border-t-black rounded-full animate-spin"></div></div>
  }

  return (
    <section className="w-full bg-gray-100 pt-10 px-3 sm:px-5 md:px-20 pb-10">
      <h1 className="font-montserrat font-semibold text-lg">Just For You</h1>
